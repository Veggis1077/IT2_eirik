import pygame
pygame.init()
karakterer = "/Users/vegard_ragnhildstveit/Desktop/IT2/King_Jump/char_blue.png"
class Game:
    def __init__(self):
    ...
    def hopper(self, lengde, a):
        if float(lengde) < 0.5:
            for i in range(2):
                self.running(self.sprite_sheet_image, 3+ int(i), self.counter)
    ...
    def game_loop(self):
        ...
        plusse = 0
        counter = 1
        b = self.running(self.sprite_sheet_image, 3, self.counter)  # Løpende figur
        c = self.running(self.sprite_sheet_image, 2, self.counter)  # Slå figur
        while run:
            # Update background
            ...
            if time_elapsed_since_last_action > 100:
                if self.hopp:
                    ...
                    if counter > 16:
                        ...
                        counter = 1
                    if counter <=8:
                        b= self.running(self.sprite_sheet_image, 4, counter%8)
                    else:
                        b = self.running(self.sprite_sheet_image, 5, counter %8)



                elif self.lope:
                    b = self.running(self.sprite_sheet_image, 3, self.counter % 8)
                elif not self.lope and not self.hopp:
            ...
            if self.hoyre:
                self.screen.blit(b, (x_pos, y_pos + plusse))
            else:
                """Her løper du til venstre"""
                self.screen.blit(pygame.transform.flip(b, True, False), (x_pos, y_pos))
            self.screen.blit(c, (200, 200))
